...
main:
  ...
  la $a0, array	# pega endereço de array e joga no endereço a0
  
  la $t0, size # pega endereço asize e joga no endereço t0
  lw $a1, 0($t0) # carrega o valor que esta em T0
  
  jal heapsort #chama a função heapsort
...
heapsort: # a0 = &array, a1 = size(array)
  addi $sp, $sp, -12
  sw $a1, 0($sp) #salvo o valor contido em a1 no endereço (sp -12) + 0
  sw $a2, 4($sp) #salvo o valor contido em a1 no endereço (sp -12) + 4
  sw $ra, 8($sp) #salvo o valor contido em a1 no endereço (sp -12) + 8, Ra, o endereço da linha anterior, que seria no main
...
make_heap: # a0 = &array, a1 = size
  ...
  sw $a1, 0($sp) #salvo o valor contido em a1 no endereço (sp -12) + 0
  sw $a2, 4($sp) #salvo o valor contido em a1 no endereço (sp -12) + 4
  sw $ra, 8($sp) #salvo o valor contido em a1 no endereço (sp -12) + 8 , Ra, o endereço da linha anterior que seria a função anterior
  
  addi $a2, $a1, -1  # a2 = size - 1
  
  addi $a1, $a1, -1  # start_index = size - 1
  srl $a1, $a1, 1  # desloca os bits, ex : 1001 fica 0100
  #blt menor, ble menor igual
  blt $a1, $zero, end_make_heap  # if(a1 (start_index < 0) return
...
bubble_down: # a0 = &array, a1 = s_idx, a2 = end
  move $t0, $a1  # movo valor de a1 para t0, , t0 recebe o start_index
  sll $t1, $t0, 1  # move para esquerda e salva em t1
  addi $t1, $t1, 1 #soma t1 mais 1
  
  bgt $t1, $a2, end_bubble_down #if ( t1 > a2) end_bubble_down
...
end_bubble_down:
  jr $ra #volto para a linha no qual inicializei a função
  
.data
# the array that holds unordered data
array: .word 5,7,1,3,6
size: .word 5
space: .asciiz " "
newline: .asciiz "\n"